...
public class AVLTree<E extends Comparable<E>> {
    ...
    private void rotateLR(Node<E> n) {
        Node<E> A, B,C, T1, T2, T3,T4, p; // p is C's parent ; note we never use T1 or T4!
        ...
        // making B the root of the subtree
        if (C == root) { // special case: C was the root of the *whole* tree
            root = B;
            B.parent = null;

        } else { // otherwise, C was just a node in the tree, not its root
            if (p.left == C) {
                p.left = B;
            } else {
                p.right = B;
        ...
        // now let's make C into B's right subchild and A into B's left subchild
        B.right = C;
        C.parent = B;
        ...
        // finally, let's move T2 to A's new right subchild and T3 to C's left subchild
        A.right = T2;
        if (T2 != null) {
            T2.parent = A;
        }
    ...
    private void rotateRL(Node<E> n) {
        ...
        // making B the root of the subtree
        if (A == root) { // special case: C was the root of the *whole* tree
            root = B;
            B.parent = null;
